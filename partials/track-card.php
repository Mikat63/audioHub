<div tabindex="0" class="track flex flex-row items-center p-2 gap-6 w-full hover:scale-105 focus:scale-105  sm:w-[40%] lg:w-[30%]" data-id="<?= htmlspecialchars(strip_tags($idTrack ?? '')) ?>" data-cover="<?= htmlspecialchars(strip_tags($coverSrc ?? 'assets/icons/default-album.svg')) ?>" data-album="<?= htmlspecialchars(strip_tags($album ?? '')) ?>" data-title="<?= htmlspecialchars(strip_tags($title ?? '')) ?>" data-artist="<?= htmlspecialchars(strip_tags($artist ?? '')) ?>" data-audiosrc="<?= htmlspecialchars(strip_tags($audioSrc ?? '')) ?>">

    <!-- cover tracks -->
    <div class="w-12 h-12 shrink-0">
        <img src="<?= htmlspecialchars(strip_tags($coverSrc ?? 'assets/icons/default-album.svg')) ?>" alt="Pochette de l'album de <?= htmlspecialchars(strip_tags($album ?? '')) ?>" loading="lazy">
    </div>
