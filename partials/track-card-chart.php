<div tabindex="0" class="track flex flex-row items-center gap-6 w-full hover:scale-105 focus:scale-105 sm:w-[80%] md:w-[70%] xl:w-[60%]" data-id="<?= htmlspecialchars(strip_tags($idTrack)) ?>" data-cover="<?= htmlspecialchars(strip_tags($coverSrc)) ?>" data-album="<?= htmlspecialchars(strip_tags($album)) ?>" data-title="<?= htmlspecialchars(strip_tags($title)) ?>" data-artist="<?= htmlspecialchars(strip_tags($artist)) ?>" data-audiosrc="<?= htmlspecialchars(strip_tags($audioSrc)) ?>">
    <!-- number position -->
